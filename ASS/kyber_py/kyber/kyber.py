...
class Kyber:
    def __init__(self, parameter_set):
        ...
        self.eta_3 = parameter_set["eta_3"]
        self.eta_4 = parameter_set["eta_4"]

        self.M = Module(self.n)
        self.R = self.M.ring
    ...
    def _cpapke_enc(self, pk, m, coins):
        ...
        r, N = self._generate_error_vector(coins, self.eta_3, N)
        r_hat = r.to_ntt()

        # Generate the error vector e1 ∈ R^k
        e1, N = self._generate_error_vector(coins, self.eta_4, N)

        # Generate the error polynomial e2 ∈ R
        e2, N = self._generate_polynomial(coins, self.eta_4, N)
    ...
    def _cpapke_enc_list(self, pk, m, coins):
        ...
        c_list = [v.coeffs] + [u._data[i][0].coeffs for i in range(len(u._data))]
        return c_list
    ...
    def _cpapke_dec_list(self, sk, c_list):
        v = self.R(c_list[0])
        u = self.M([[self.R(ci)] for ci in c_list[1:]])
        c1 = u.encode(12)
        c2 = v.encode(12)
        return self._cpapke_dec(sk, c1 + c2)
